	...
	<div class="row">
		<div class="col-sm-4">
			<div class="card card-primary mb-3">
				...
				<div class="card-body">
					<input type="text" id="cari" class="form-control" name="cari" placeholder="Masukan : Kode / Nama produk  [ENTER]">
				</div>
			</div>
		</div>
		<div class="col-sm-8">
								...
								<tr>
									...
									<td>
										...
										<form method="POST" action="fungsi/edit/edit.php?jual=jual">
												...
												<input type="hidden" name="id_produk" value="<?php echo $isi['id_produk'];?>" class="form-control">
											</td>
										...
										<!-- aksi ke table penjualan -->
										<a href="fungsi/hapus/hapus.php?jual=jual&id=<?php echo $isi['id_penjualan'];?>&brg=<?php echo $isi['id_produk'];?>
							...
							if(!empty($_GET['nota'] == 'yes')) {
								...
								if(!empty($bayar))
								{
									...
									if($bayar >= $total)
									{
										...
										$jumlah_dipilih = count($id_produk);
										
										for($x=0;$x<$jumlah_dipilih;$x++){

											$d = array($id_produk[$x],$id_member[$x],$jumlah[$x],$total[$x],$tgl_input[$x],$periode[$x]);
											...
											$row_produk->execute(array($id_produk[$x]));
											$hsl = $row_produk->fetch();
											...
											// echo $total_stok;
											$sql_stok = "UPDATE produk SET stok = ? WHERE id_produk = ?";
...
$(document).ready(function(){
	$("#cari").change(function(){
		$.ajax({
			type: "POST",
			url: "fungsi/edit/edit.php?cari_produk=yes",
