...
class view
{
    ...
    public function produk_cari($cari)
    {
        $sql = "select produk.*, kategori.id_kategori, kategori.nama_kategori
                from produk inner join kategori on produk.id_kategori = kategori.id_kategori
                where id_produk like '%$cari%' or nama_produk like '%$cari%' or merk like '%$cari%'";
        $row = $this-> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetchAll();
        return $hasil;
    ...
    public function produk_id()
    {
        ...
        $urut = substr($hasil['id_produk'], 2, 3);
        $tambah = (int) $urut + 1;
        if (strlen($tambah) == 1) {
            $format = 'BR00'.$tambah.'';
    ...
    public function produk_stok_row()
    {
    ...
    public function produk_beli_row()
    {
        $sql ="SELECT SUM(harga_beli) as beli FROM produk";
        $row = $this-> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetch();
        return $hasil;
    ...
    {
        $sql ="SELECT nota.* , produk.id_produk, produk.nama_produk, produk.harga_beli, member.id_member,
    ...
    public function jml()
    {
        $sql ="SELECT SUM(harga_beli*stok) as byr FROM produk";
        $row = $this -> db -> prepare($sql);
        $row -> execute();
        $hasil = $row -> fetch();
        return $hasil;
