...
if (!empty($_SESSION['admin'])) {
    ...
    if (!empty($_GET['stok'])) {
        ...
        $id = htmlentities($_POST['id']);
        $dataS[] = $id;
        $sqlS = 'select*from produk WHERE id_produk=?';
    ...
    if (!empty($_GET['produk'])) {
        ...
        $data[] = $tgl;
        $data[] = $id;
        $sql = 'UPDATE produk SET id_kategori=?, nama_produk=?, merk=?, 
				harga_beli=?, harga_jual=?, satuan_produk=?, stok=?, tgl_update=?  WHERE id_produk=?';
        $row = $config -> prepare($sql);
        $row -> execute($data);
        echo '<script>window.location="../../index.php?page=produk/edit&produk='.$id.'&success=edit-data"</script>';
    }
    ...
    if (!empty($_GET['jual'])) {
        $id = htmlentities($_POST['id']);
        $id_produk = htmlentities($_POST['id_produk']);
        $jumlah = htmlentities($_POST['jumlah']);

        $sql_tampil = "select *from produk where produk.id_produk=?";
        $row_tampil = $config -> prepare($sql_tampil);
        $row_tampil -> execute(array($id_produk));
        ...
        if ($hasil['stok'] > $jumlah) {
            ...
            echo '<script>alert("Keranjang Melebihi Stok produk Anda !");
    ...
    if (!empty($_GET['cari_produk'])) {
        ...
        if ($cari == '') {
            ...
            $sql = "select produk.*, kategori.id_kategori, kategori.nama_kategori
					...
					where produk.id_produk like '%$cari%' or produk.nama_produk like '%$cari%' or produk.merk like '%$cari%'";
            $row = $config -> prepare($sql);
            $row -> execute();
            $hasil1= $row -> fetchAll();
            ?>
		...
		<?php foreach ($hasil1 as $hasil) {?>
			<tr>
				...
				<td>
				<a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_produk'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>" 
